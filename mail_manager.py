...
def recreate_db():
    ...
    conn = sqlite3.connect('email_accounts.db')
    cursor = conn.cursor()
    ...
    conn.commit()
    conn.close()
    print("Database table recreated with the updated schema.")
...
class EmailAccountManager:
    def __init__(self):
        self.conn = sqlite3.connect('email_accounts.db', check_same_thread=False)
    ...
    def set_master_password(self):
        ...
        while True:
            master_password = getpass.getpass("Set a master password (input is hidden): ").strip()
            if master_password:
    ...
    def change_master_password(self):
        ...
        old_password = getpass.getpass("Enter current master password (input is hidden): ").strip()
        if old_password != self.master_password:
            print("Incorrect master password!")
            return False
        
        new_password = getpass.getpass("Enter new master password (input is hidden): ").strip()
        if not new_password:
        ...
        self.cursor.execute('SELECT id, password_encrypted, twitter_password_encrypted FROM accounts')
        ...
        for account in accounts:
            ...
            try:
                ...
                if twitter_password_encrypted:
                    twitter_password = decrypt_password(twitter_password_encrypted, self.encryption_key)
                else:
    ...
    def add_account(self, email, github_username=None, twitter_username=None, twitter_password=None):
        """Add a new account."""
        ...
        ''', (email, password_encrypted, github_username, twitter_username, twitter_password_encrypted))
        self.conn.commit()
    ...
    def update_password(self, email, new_password=None):
        ...
        self.cursor.execute('UPDATE accounts SET password_encrypted = ? WHERE email = ?', (password_encrypted, email))
        self.conn.commit()
    ...
    def list_accounts(self, show_passwords=False):
        ...
        for account in accounts:
            (
                account_id, email, password_encrypted, github_username,
                twitter_username, twitter_password_encrypted
            ) = account
            ...
            if show_passwords:
                ...
                try:
                    password_display = decrypt_password(password_encrypted, self.encryption_key)
                except Exception as e:
                ...
                if twitter_username:
                    try:
                        twitter_password_display = (
                            decrypt_password(twitter_password_encrypted, self.encryption_key)
                            if twitter_password_encrypted
    ...
    def update_github_username(self, email, github_username):
        ...
        self.cursor.execute('UPDATE accounts SET github_username = ? WHERE email = ?', (github_username, email))
        self.conn.commit()
    ...
    def update_twitter_username(self, email, twitter_username):
        ...
        self.cursor.execute('UPDATE accounts SET twitter_username = ? WHERE email = ?', (twitter_username, email))
        self.conn.commit()
    ...
    def update_twitter_password(self, email, twitter_password=None):
        ...
        twitter_password_encrypted = encrypt_password(twitter_password, self.encryption_key)
        self.cursor.execute('UPDATE accounts SET twitter_password_encrypted = ? WHERE email = ?', (twitter_password_encrypted, email))
        self.conn.commit()
        print(f"Twitter password updated successfully! New password: {twitter_password}")
        return True
...
# Main menu
def main_menu():
    # recreate_db()  # Drop and recreate the table
    ...
    while True:
        ...
        if choice == '1':
            ...
            if '@' not in email:
                ...
                continue
            github_username = get_input("Enter associated GitHub username (optional, or 'q' to return): ") or None
            if github_username is None:
                continue
            twitter_username = get_input("Enter associated Twitter username (optional, or 'q' to return): ") or None
            if twitter_username is None:
                continue
            if twitter_username:
                twitter_password = getpass.getpass("Enter Twitter password (optional, default to email password, or 'q' to return): ") or None
                if twitter_password is None:
                    continue
            else:
                twitter_password = None
            manager.add_account(email, github_username, twitter_username, twitter_password)
            ...
            if email is None:
                continue
            action = get_input("Generate a new password automatically? (y/n, or 'q' to return): ")
            if action is None:
                continue
            if action.lower() == 'y':
            ...
            master_pwd = getpass.getpass("Enter master password to view all account passwords (or 'q' to return): ")
            ...
            email = get_input("Enter email to update GitHub username (or 'q' to return): ")
            if email is None:
                continue
            github_username = get_input("Enter new GitHub username (or 'q' to return): ")
            if github_username is None:
                continue
            manager.update_github_username(email, github_username)

        elif choice == '8':
            email = get_input("Enter email to update Twitter username (or 'q' to return): ")
            if email is None:
                continue
            twitter_username = get_input("Enter new Twitter username (or 'q' to return): ")
            if twitter_username is None:
                continue
            manager.update_twitter_username(email, twitter_username)

        elif choice == '9':
            email = get_input("Enter email to update Twitter password (or 'q' to return): ")
            if email is None:
                continue
            action = get_input("Generate a new password automatically? (y/n, or 'q' to return): ")
            if action is None:
            ...
            if action.lower() == 'y':
                ...
                new_pwd = getpass.getpass("Enter new Twitter password (or 'q' to return): ")
                if new_pwd is None:
                    continue
